...
export const createHoliday = async (req, res) => {
    try {
        ...
        const holiday = new HolidayCalendar({ date, reason });
        await holiday.save();
        res.status(201).json({ message: 'HolidayCalendar created successfully', holiday });
    } catch (error) {
...
export const updateHoliday = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'HolidayCalendar updated successfully', holiday });
    } catch (error) {
        res.status(400).json({ error: error.message });
    }
};
